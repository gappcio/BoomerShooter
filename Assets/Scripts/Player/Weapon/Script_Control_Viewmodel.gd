...
func _process(delta):
	...
	&& can_shoot:
		state_action = WEAPON_ACTION_STATE.shoot;
		AnimTreeActionArm["parameters/playback"].travel("shoot");
		AnimTreeActionWeapon["parameters/playback"].travel("shoot");
		can_shoot = false;
...
func animate():
	...
	match(state_action):
		WEAPON_ACTION_STATE.ref:
			...
			AnimTreeActionWeapon["parameters/playback"].travel("idle");
